...
6. ~~**B2 — Admin area + Implementation Tracker**~~ — **DONE** (2026‑09‑22). Learning progress retired; `/admin` tracker + `ADMIN_USERNAME` gate in; A1 suite reworked self‑contained (13/13), A2 re‑verified (18/18 incl. live).
7. **B3 — Comic editor (upload + reorder + captions + live preview + auto‑save)** — **NEXT (now).**
8. **B4 — Comic marquee / crop selection** (rectangle select → confirm → only that region shows; preview toggle; re‑marquee prompt).
9. **B5 — Story editor (upload / paste / Google Doc link + auto‑save).**
10. **B6 — Story archive import** (read an archive doc's links → stories; download a story + images; global button).
11. **B7 — Story tier‑gating** (inline highlight → per‑tier blur + theme‑red premium border; hide/show blurred sections).
12. **B8 — Public display + member gating** (comic reader, story pages with B7 highlights, gallery, video; enforce is_member + tier; copy‑prevention).
13. **B9** — real storage (deferred until members arrive) · **C1+** — Hostinger/domain/CDN **when we're ready to leave the free tier.**

> **We are NOT touching Hostinger (C1).** A1 + A2 + B0 (scaffold + EJS) + B1 (content DB + red/black theme) are done —
> the next build step is **B2 (admin area + implementation tracker)**, then the editors B3 → B7, then display B8. A3–A5 ride along with Phase B.
>
...
> (`gqsa/pen-name-site`). As of B2 it is **4 commits behind** local main (running the A2‑hotfix #5 code —
> pre‑EJS, pre‑content‑DB), because B0–B2 haven't been pushed yet. Guardrails on every step: (1) a git
> commit (rollback available) and (2) both suites green — `test-a1-security.mjs` (**13/13**, self‑contained)
> + `test-a2-subscriptions.mjs` (**18/18**, incl. live section). B‑steps are additive (new routes/tables/
> pages), not changes to the A1/A2 core (auth + PayPal), so pushing is low‑risk; a push triggers a Render
> auto‑deploy and — per C3 — wipes the ephemeral DB/uploads (re‑upload test content after). Push when ready.
